...
bool x_placement[9] = {false};

bool o_placement[9] = {false};

//tells which mode of game is being played
int Mode = 0;

//tells which player is computer 
int Computer = 0;
...
int* computer_move(char arena[][3],char placeholder)
{
    int arr[2] = {0};
    if(empty_count == 0)
    {
    ...
    else
    {
        if(placeholder == 'x')
        {
            //check win conditions of o
        }
        else if (placeholder == 'o')
        {
            //check win conditions of x
        }
    }
}
void arena_printer(char arena[][3])
...
int main()
{
    printf("Choose Which mode do you want to play ? \n 1.Two Player Mode \n 2.Single Player Mode \nOption :");
    int mode = 0;
    while(1)
    {
        scanf(" %d",&mode);
